	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			try {
				...
				String name = null;
				String artType = null;
				String artPrice = null;
				String uploadedBy=null;
				for (FileItem item : multiparts) {
					if (!item.isFormField()) {
						name = new File(item.getName()).getName();
						item.write(new File(UPLOAD_DIRECTORY + File.separator + name));
						FileItem art_type=(FileItem) multiparts.get(0);
						artType = art_type.getString();
						System.out.println("Art Type "+artType);
						FileItem price=(FileItem) multiparts.get(1);
						artPrice=price.getString();
						System.out.println("Art Price "+artPrice);
					}
				}
				try {
					int id = 0;
					...
					int i = st.executeUpdate("insert into room values('" + id + "','" + artType + "','" + artPrice + "','"+name+"','"+imagePath+"')");
					if (i > 0) {
